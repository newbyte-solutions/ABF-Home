  <template>
    <div class="p-5">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-7xl mx-auto">
        <div v-for="article in filteredNews" :key="article._id" class="bg-white rounded-lg p-5 shadow-md">
          <h3 class="mt-0 mb-2.5">{{ article.articleTitle }}</h3>
          <p>{{ article.articleDescription }}</p>
          <span class="text-gray-600 text-sm">{{ formatDate(article.articlePublishedDate) }}</span>
        </div>
      </div>
    </div>
  </template>
...
<script setup>
  ...
  const fetchNews = async () => {
    ...
    try {
      ...
      articles.value = response.data.filter(item => item.articleCompany === props.companyId)
