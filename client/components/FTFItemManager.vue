<template>
  <div class="bg-white rounded-lg shadow p-6 max-w-xl mx-auto">
    <h2 class="text-2xl font-bold mb-4">Administrer status og tilgjengelighet</h2>
    <form v-if="item" @submit.prevent="submitManage" class="space-y-4">
      ...
      <div>
        ...
        <input v-model="item.currentUser" type="text" class="input" placeholder="Bruker-ID eller navn" />
      </div>
      <div>
        <label class="block font-medium mb-1">Legg til logg (reparasjon, status, etc)</label>
        <textarea v-model="logEntry" class="input" placeholder="Skriv logginnlegg her..." />
      </div>
    ...
    <div v-if="item && item.logs && Array.isArray(item.logs) && item.logs.length" class="mt-6">
...
async function submitManage() {
  ...
  try {
    ...
    if (logEntry.value) {
      ...
      item.value.logs.push(`[${new Date().toLocaleString()}] ${logEntry.value}`)
    }
